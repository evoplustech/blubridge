...
const NetworkCanvas = () => {
  ...
  useEffect(() => {
    ...
    //   requestAnimationFrame(draw);
    // };
    const basePoints = points.current.map(([x, y]) => ({ x, y, phaseX: Math.random() * Math.PI * 2, phaseY: Math.random() * Math.PI * 2 }));
    ...
    const draw = () => {
        ...
        animatedPoints.forEach((point, idx) => {
            const neighbors = animatedPoints
            .map((p, i) => ({ index: i, distance: getDistance(point, p) }))
            .filter((n) => n.index !== idx)
            .sort((a, b) => a.distance - b.distance)
            .slice(0, NEIGHBOR_COUNT);
            ...
            neighbors.forEach(({ index }) => {
            const neighbor = animatedPoints[index];
            const key = `${idx}-${index}`;
            let targetOpacity = BASE_OPACITY;
